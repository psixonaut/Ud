...
class Test_drive(models.Model):
    ...
    def clean(self):
        ...
        if hasattr(self, 'vin') and self.datetime_reservation:
            if Test_drive.objects.filter(
                    vin=self.vin,
                    datetime_reservation=self.datetime_reservation
            ).exclude(pk=self.pk).exists():
                raise ValidationError(f"Автомобиль {self.vin} уже занят на это время.")

        if hasattr(self, 'passport_client') and self.datetime_reservation:
            if Test_drive.objects.filter(
                    passport_client=self.passport_client,
                    datetime_reservation=self.datetime_reservation
            ).exclude(pk=self.pk).exists():
                raise ValidationError(f"Клиент {self.passport_client.fio} уже записан на тест-драйв в это время.")

        if hasattr(self, 'id_employee') and self.datetime_reservation:
            if Test_drive.objects.filter(
                    id_employee=self.id_employee,
                    datetime_reservation=self.datetime_reservation
            ).exclude(pk=self.pk).exists():
                raise ValidationError(f"Сотрудник {self.id_employee.fio} уже проводит тест-драйв в это время.")
        ...
        if self.pk:
            ...
            if old_obj.datetime_reservation != self.datetime_reservation:
                ...
                if days_diff < 2:
                    raise ValidationError("Нельзя перенести тест-драйв на дату, до которой осталось меньше 2 дней.")
